...
from .moduleY import NetY
from .moduleT import NetT
...
tf.app.flags.DEFINE_string('name', 'IPWNet', """Model Name """)
...
class IPWNet(object):
    ...
    def train(self, exp, dataName, testDB, FLAGS):
        ...
        ''' Define model graph '''
        TNN = NetT(n, x_dim, FLAGS)
        D_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='treatment')
        opt_TNN = tf.train.AdamOptimizer(0.001)
        train_TNN = opt_TNN.minimize(TNN.cost_loss,var_list=D_vars)
    
        ''' Define model graph '''
        REP = NetY(n, x_dim, FLAGS)
        ...
        train_REP = opt.minimize(REP.tot_loss,global_step=global_step,var_list=O_vars)
        ...
        log(logfile, f'Experiemnts({exp}) - Treatment Regression... ')
        for i in range(1000):
            ...
            sess.run(train_TNN, feed_dict={TNN.tnet_x: x_batch, TNN.tnet_t: t_batch})
        ...
        objnan = False
        early_stop = 9999
        log(logfile, f'Experiemnts({exp}) - Outcome Regression... ')
        for i in range(FLAGS.iterations):
            ...
            ''' Do one step of gradient descent '''
            if not objnan:
                w_batch = sess.run(TNN.ipw, feed_dict={TNN.tnet_x: x_batch, TNN.tnet_t: t_batch})
                sess.run(train_REP, feed_dict={REP.x: x_batch, REP.t: t_batch, REP.ipw: w_batch, \
            ...
            if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
                obj_loss,f_error,imb_err = sess.run([REP.total_loss, REP.pred_loss, REP.imb_dist],feed_dict=dict_train)
                valid_obj, valid_f_error, valid_imb = sess.run([REP.total_loss, REP.pred_loss, REP.imb_dist], feed_dict=dict_valid)
