...
class NetY(object):
    def __init__(self, n, x_dim, FLAGS):
        ...
        self.ipw= tf.placeholder("float", shape=[None, 1], name='ipw')   # IPW
    ...
    def _build_graph(self, FLAGS):
        ...
        if FLAGS.loss == 'l1':
            ...
            totrisk = tf.reduce_mean(tf.abs(y_-y))
            ...
            totrisk = -tf.reduce_mean(res)
            pred_error = -tf.reduce_mean(res)
        else:
            risk = tf.reduce_mean(sample_weight*tf.square(y_ - y))
            totrisk = tf.reduce_mean(tf.square(y_ - y))
        ...
        if FLAGS.p_alpha>0:
            ...
            total_error = total_error + imb_error

        if FLAGS.p_lambda>0:
            tot_error = tot_error + r_lambda*self.wd_loss
            total_error = total_error + r_lambda*self.wd_loss
