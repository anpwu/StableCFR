...
class PM25(object):
    ...
    def run(self, config=None):
        ...
        y_temp = df_tyx['CMR_2010'].values / 50 
        df_tyx = df_tyx.drop(['CMR_2010'], axis=1)
        pi = copy.deepcopy(df_tyx['PM2.5_2010'].values)
        df_tyx['FIPS'] = pi
        t  = (pi > self.t_thres).astype(np.float64)
        df_tyx['PM2.5_2010'] = t
        ...
        if self.num < len(df_tyx):
            ...
            self.num = len(df_tyx)
        print('Generate Datasets: {}({}_{}). '.format(self.name, self.t_thres, self.num))
        data_numpy = df_tyx.values

        for exp in range(self.reps):
            self.genTrain(exp, data_numpy, y_temp)

        show_path = './Data/{}({}_{})/draw/'.format(self.name, self.t_thres, self.num)
        os.makedirs(os.path.dirname(show_path), exist_ok=True)
        bottom, top = self.show(show_path, df_tyx, self.drawif)

    def genTrain(self, exp, data_numpy, y_temp):
        num = self.num
        ...
        data_path = './Data/{}({}_{})/{}/'.format(self.name, self.t_thres, self.num, exp)
